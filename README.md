...
LinkX 是一款基于 **Vue 3 + Electron** 构建的跨平台即时通讯桌面应用，提供消息、联系人、收藏、文件、友链等核心模块的前端交互演示，后续将对接 Java 后端服务。
...
> 当前版本消息等模块仍使用本地 Mock 数据；**用户认证已对接 Java 后端**（登录 / 注册 / Refresh Token 自动登录）。
...
| `SPRING_PROFILES_ACTIVE` | 否 | 默认 `local`（读取 `application-local.yml`） |

本地开发：复制 `linkx-server/src/main/resources/application-local.yml.example` 为 `application-local.yml`（已 gitignore）。
...
通过环境变量配置后端地址（参见 `linkx-client/.env.example`）：
...
生产部署时在后端 `application-local.yml` 或环境变量中配置 HTTPS CORS 白名单：
...
- [x] 实现用户认证与双 Token 管理
- [x] 对接 WebSocket 实时消息（Netty，文本/图片/文件）
