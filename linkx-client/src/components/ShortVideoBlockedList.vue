...
import {
  listBlockedShortVideoAuthors,
  unblockShortVideoAuthor,
  type ShortVideoBlockedUser
} from '../api/shortVideo'
...
const emit = defineEmits<{
  close: []
  select: [user: ShortVideoBlockedUser]
  unblock: [userId: string]
}>()

const { t } = useI18n()

const users = ref<ShortVideoBlockedUser[]>([])
...
async function loadUsers(reset = false) {
  ...
  try {
    const beforeId = reset ? undefined : users.value[users.value.length - 1]?.blockId
    const res = await listBlockedShortVideoAuthors({ beforeId, limit: pageSize })
    if (res.code !== 200) {
      throw new Error(res.message || 'load blocked users failed')
    }
    const rows = Array.isArray(res.data) ? res.data : []
    if (reset) {
      users.value = rows
    } else {
      const existing = new Set(users.value.map(item => item.blockId))
      users.value.push(...rows.filter(item => !existing.has(item.blockId)))
    }
    hasMore.value = rows.length >= pageSize
  } catch (e) {
    error.value = resolveApiErrorMessage(e, t('shortVideo.blockedListLoadFail'))
...
function pickUser(user: ShortVideoBlockedUser) {
  emit('select', user)
}

async function unblock(user: ShortVideoBlockedUser, event: Event) {
  event.stopPropagation()
  if (!user.userId || unblockingId.value) return
  unblockingId.value = user.userId
  try {
    const res = await unblockShortVideoAuthor(user.userId)
    if (res.code !== 200) {
      throw new Error(res.message || 'unblock failed')
    }
    users.value = users.value.filter(item => item.userId !== user.userId)
    emit('unblock', user.userId)
  } catch (e) {
    error.value = resolveApiErrorMessage(e, t('shortVideo.unblockAuthorFail'))
  } finally {
    unblockingId.value = ''
  }
}
...
<template>
  <ShortVideoSubPageShell
    v-if="open"
    elevated
    :title="t('shortVideo.blockedAuthors')"
  ...
  >
    <div class="sv-following-scroll" @scroll="onScroll">
      ...
      <div v-else-if="isEmpty" class="sv-subpage__empty">{{ t('shortVideo.noBlockedAuthors') }}</div>
      <ul v-else class="sv-following-list">
        <li v-for="user in users" :key="user.blockId" class="sv-following-list__item">
          ...
          <button
            ...
            :disabled="Boolean(unblockingId && unblockingId !== user.userId)"
            @click="unblock(user, $event)"
          >
            {{ unblockingId === user.userId ? '…' : t('shortVideo.unblockAuthor') }}
          </button>
        </li>
      </ul>
