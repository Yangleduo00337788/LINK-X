...
import {
  ...
  listShortVideoFollowerUsers,
  unfollowShortVideoAuthor,
  type ShortVideoFollowerUser
} from '../api/shortVideo'
...
const emit = defineEmits<{
  close: []
  select: [user: ShortVideoFollowerUser]
  followChange: [userId: string, following: boolean]
}>()

const { t } = useI18n()

const users = ref<ShortVideoFollowerUser[]>([])
...
const panelTitle = computed(() => props.title?.trim() || t('shortVideo.followerListTitle'))
...
async function loadUsers(reset = false) {
  ...
  try {
    ...
    const res = await listShortVideoFollowerUsers(userId, { beforeId, limit: pageSize })
    if (res.code !== 200) {
      throw new Error(res.message || 'load follower users failed')
    }
    ...
    error.value = resolveApiErrorMessage(e, t('shortVideo.followerListLoadFail'))
...
function pickUser(user: ShortVideoFollowerUser) {
  emit('select', user)
}

async function toggleFollow(user: ShortVideoFollowerUser, event: Event) {
...
<template>
  ...
  >
    <div class="sv-following-scroll" @scroll="onScroll">
      ...
      <div v-else-if="isEmpty" class="sv-subpage__empty">{{ t('shortVideo.noFollowers') }}</div>
