...
**注意：** 每个角色使用独立账号，勿与真实运营账号混用。抽检前确认 Flyway 已执行至 `V47+`。
...
- [ ] 登录成功，侧边栏含：系统配置、菜单管理、角色管理、IP 限流
...
| 运营看不到推荐/活动 | `ops_admin` 未绑定 `admin:recommend:*` / `admin:activity:*` |
