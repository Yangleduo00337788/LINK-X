...
public interface CloudShareMapper extends BaseMapper<CloudShare> {
    ...
    /**
     * 原子递增分享下载次数，仅当未超限时才更新成功（返回 1）。
     * max_downloads 为 NULL 视为不限次数；超限时返回 0。
     * 通过受影响行数判断是否超限，避免 check-then-set TOCTOU 竞态。
     */
    @Update("UPDATE cloud_share SET download_count = download_count + 1, update_time = NOW() " +
            "WHERE id = #{id} AND (max_downloads IS NULL OR download_count < max_downloads)")
