    ...
    public static List<SnailJobJobDefinition> all() {
        return List.of(
                ...
                cron("message_retention", "消息留存清理",
                        "逻辑删除超过保留期的历史消息并清理附件",
                        "0 0 3 * * ?", 1800),
                fixed("message_content_reencrypt", "内容历史重加密",
                        "将开启落库加密前的明文 IM 消息与朋友圈内容批量转为密文（加密关闭时自动跳过）",
                        300, 900),
                fixed("message_content_key_rotate", "内容密钥轮换",
                        "将旧 keyId 密文批量重加密为当前 MESSAGE_KEK_KEY_ID（含 IM 与朋友圈；加密关闭时自动跳过）",
                        300, 900),
                fixed("search_text_backfill", "搜索摘要回填",
                        "为历史 IM 消息与朋友圈动态回填 search_text（解密后生成检索摘要，供 FULLTEXT 搜索）",
                        300, 900),
