    ...
    public static List<SnailJobJobDefinition> all() {
        return List.of(
                ...
                cron("message_retention", "消息留存清理",
                        "逻辑删除超过保留期的历史消息并清理附件",
                        "0 0 3 * * ?", 1800),
                fixed("message_content_reencrypt", "消息历史重加密",
                        "将开启落库加密前的明文消息批量转为密文（加密关闭时自动跳过）",
                        300, 900),
                fixed("message_content_key_rotate", "消息密钥轮换",
                        "将旧 keyId 密文批量重加密为当前 MESSAGE_KEK_KEY_ID（加密关闭时自动跳过）",
                        300, 900),
